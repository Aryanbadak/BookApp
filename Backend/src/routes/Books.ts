...
router.post('/', upload.single('cover'), async (req, res) => {
  try {
    ...
    const [result] = await pool.query(
      'INSERT INTO books (title, author, year, genre, cover_path) VALUES (?, ?, ?, ?, ?)',
      [title, author, year || null, genre || null, cover_path]
    );

    res.json({ id: (result as any).insertId, title, author, year, genre, cover_path });
  } catch (err) {
    console.error(err);
...
router.put('/:id', async (req, res) => {
  ...
  const { title, author, year, genre } = req.body;
  let cover_path = null;
  if (req.file) cover_path = `/uploads/${req.file.filename}`;
